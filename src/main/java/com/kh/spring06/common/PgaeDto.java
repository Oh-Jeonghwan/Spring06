...
public class PgaeDto {
	...
	private boolean prev; //이전 버튼을 누를 수 있느 ㄴ경우/없는 경우 분류
	...
	private void calcData() {
		...
		next = endPage * cri.getPerPageNum() >= totalCount? false:true; //endPage가 페이지 당 최대 페이지보다 크거나 같으면 false
	}
	...
	public String toString() {
		return "PgaeDto [cri=" + cri + ", totalCount=" + totalCount + ", startPage=" + startPage + ", endPage="
				+ endPage + ", prev=" + prev + ", next=" + next + ", displayPageNum=" + displayPageNum
				+ ", tempEndPage=" + tempEndPage + "]";
	}
	
	//jsp에서 페이징 처리시 url 하드코딩을 방지하기 위한 메소드
